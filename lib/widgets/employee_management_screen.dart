...
class _EmployeeManagementScreenState extends State<EmployeeManagementScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Check if user is authenticated before loading employees
      final isAuthed = auth.isAuthenticated;
      
      if (isAuthed) {
        context.read<EmployeeProvider>().loadEmployees(refresh: true);
      } else {
        // Show info message that data requires authentication
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please sign in to access employee data'),
